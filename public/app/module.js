(function () {
  ...
  ec2pricing.factory("pricingData", function ($q, pricingUrls, jsonpLoader, cache, awsDataParser) {
    ...
    var loadData = function (url, category) {
      return cache(url, jsonpLoader, url, "callback").then(function (data) {
        data.pricingCategory = category
        data.operatingSystem = guessOsFromUrl(url)
        return data
      })
    }
    var deferreds = []
    angular.forEach(pricingUrls, function (urls, category) {
      urls.forEach(function (url) {
        var deferred = loadData(url, category)
        deferreds.push(deferred)
      })
    })
    return $q.all(deferreds).then(awsDataParser)
  })
}())
